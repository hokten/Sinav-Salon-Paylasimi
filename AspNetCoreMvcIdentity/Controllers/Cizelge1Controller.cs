  ...
  public class Cizelge1Controller
    : Controller {
      ...
      public async Task<IActionResult> SinavEkle (Sinav sinav) {
        ...
        int KullaniciBolumId = user.BolumId;
        if (!ModelState.IsValid) {
          Dictionary<string, string> UyariMesajiDict = new Dictionary<string, string> () { { "Tip", "danger" }, { "Mesaj", "Oturum zamanı veya salon bilgisi uygunsuz." } };
          TempData["UyariMesaji"] = UyariMesajiDict;
          return RedirectToAction (nameof (Index));
        }
        if (!_context.Oturum.Any (m => m.OturumId == sinav.OturumId) || !_context.Salon.Any (m => m.SalonId == sinav.SalonId)) {
          Dictionary<string, string> UyariMesajiDict = new Dictionary<string, string> () { { "Tip", "danger" }, { "Mesaj", "Oturum zamanı veya salon bulunamadı." } };
          TempData["UyariMesaji"] = UyariMesajiDict;
          return RedirectToAction (nameof (Index));
        }
        if (_context.Sinav.Any (o => o.OturumId == sinav.OturumId && o.SalonId == sinav.SalonId)) {
          Dictionary<string, string> UyariMesajiDict = new Dictionary<string, string> () { { "Tip", "danger" }, { "Mesaj", "Belirtilen oturum zamanı için ilgili salonda zaten sınav var." } };
          TempData["UyariMesaji"] = UyariMesajiDict;
          return RedirectToAction (nameof (Index));
        }
        ...
        var jkl = _context.OgretimElemani.Include(m => m.OgretimElemanininBolumleri).Select(m => new {Id = m.OgretimElemaniId, AdSoyad = m.OgretimElemaniAdiSoyadi, Bolumler = m.OgretimElemanininBolumleri.Select(c => c.BolumId).ToArray()}).Where(x => x.Bolumler.Contains(KullaniciBolumId));
        ViewData["GozetmenId"] = new SelectList (_context.OgretimElemani.Include(m => m.OgretimElemanininBolumleri).Select(m => new {Id = m.OgretimElemaniId, AdSoyad = m.OgretimElemaniAdiSoyadi, Bolumler = m.OgretimElemanininBolumleri.Select(c => c.BolumId).ToArray()}).Where(x => x.Bolumler.Contains(KullaniciBolumId)), "Id", "AdSoyad");

        //ViewData["GozetmenId"] = new SelectList (_context.OgretimElemani.Include(m => m.OgretimElemanininBolumleri).Where(m => m.OgretimElemanininBolumleri.Select(x => x.BolumId).Contains(KullaniciBolumId)), "OgretimElemaniId", "OgretimElemaniId");
      ...
      public async Task<IActionResult> Index () {
        ViewBag.UyariMesaji = TempData["UyariMesaji"] ?? (new Dictionary<string, string> ());
        var user = await _userManager.Users.Include (x => x.Bolum).SingleAsync (x => x.Id == Convert.ToInt32 (_userManager.GetUserId (User)));
        if (user == null) {
